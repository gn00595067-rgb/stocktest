...
# 批次預抓即時報價（TWSE MIS 一次多檔）：warm 同一份快取，讓下方逐檔取價「即時、一致、快速」，
# 避免逐檔序列抓取時部分未回而顯示上一輪舊值。並提供「更新報價」強制刷新與報價時間顯示。
from services.price_service import get_quotes_cached, clear_quote_cache
_hold_sids = {str(getattr(t, "stock_id", "")).strip() for t in all_trades if getattr(t, "stock_id", None)}
_hold_sids.discard("")
_rc1, _rc2 = st.columns([1, 4])
with _rc1:
    if st.button("🔄 更新報價", key="portfolio_refresh_quotes", use_container_width=True):
        clear_quote_cache()
        st.rerun()
