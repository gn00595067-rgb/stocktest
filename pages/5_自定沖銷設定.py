            ...
            with st.expander("輔助篩選配對：現價與推薦買進（依賺賠分類）", expanded=True):
                ...
                if not buys_with_remain or current_price is None:
                    ...
                    def _cat(pct):
                        ...
                        if pct >= -20: return "中賠"
                        return "大賠"
                    # 從 session state 讀取上次勾選／沖銷股數，以計算本賣出剩餘可分配與每列動態剩餘可配
                    rec_state = st.session_state.get("rec_panel_state") or {}
                    rec_state_sell = rec_state.get(sell_id) or {}
                    total_allocated = sum(
                        int(rec_state_sell.get(bid, {}).get("沖銷股數", 0) or 0)
                        for bid, v in rec_state_sell.items()
                        if isinstance(v, dict) and v.get("勾選")
                    )
                    remaining_sell = max(0, sell_remain - total_allocated)
                    recs = []
                    for t, rem in buys_with_remain:
                        ...
                        cap = min(rem, remaining_sell)
                        prev = rec_state_sell.get(t.id) or {}
                        checked = prev.get("勾選", False)
                        qty_default = prev.get("沖銷股數", cap)
                        qty_default = min(max(0, int(qty_default) if qty_default is not None else cap), cap, rem)
                        recs.append({
                            "勾選": checked,
                            "沖銷股數": qty_default,
                            "分類": _cat(pnl_pct),
                    ...
                    if df_rec.empty:
                        ...
                        # 儲存勾選與沖銷股數到 session state，下次 run 時可動態更新剩餘可配與沖銷股數
