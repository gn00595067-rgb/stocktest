...
# 當日已實現損益：以「區間內賣出 + 歷史買進」配對（先套自定沖銷，其餘 FIFO 補配）
trade_by_id = {t.id: t for t in trades}
sells_today_ids = {t.id for t in daily if (getattr(t, "side", "") or "").strip().upper() == "SELL"}
...
realized_day = float(df["單筆損益"].fillna(0).sum()) if "單筆損益" in df.columns else 0.0
