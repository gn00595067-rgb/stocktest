...
k5.metric("總賣出金額", f"{s['總賣出金額']:,.0f}", help="股數×賣價的毛額，未扣任何費用／稅。")
k6.metric("總買進成本", f"{s['總買進成本']:,.0f}", help="股數×買價＋買進手續費（買費已計入此欄）。")
k7.markdown(
    f"<div style='font-size:13px;color:#666'>總費用（手續費＋稅）</div>"
    f"<div style='font-size:26px;font-weight:700'>{s['總費用']:,.0f}</div>"
    f"<div style='font-size:12px;color:#888'>買手續費 {s['總買手續費']:,.0f}（已含在買進成本）<br>"
    f"賣手續費＋稅 {s['總賣費及稅']:,.0f}</div>",
    unsafe_allow_html=True,
)
k8.metric("平均持有天數", f"{s['平均持有天數']:.1f} 天")

# 對帳說明：買費已含在買進成本，故淨損益只需再扣「賣手續費＋稅」
st.caption(
    f"對帳：總賣出金額 {s['總賣出金額']:,.0f} − 總買進成本（含買費）{s['總買進成本']:,.0f} "
    f"− 賣手續費＋稅 {s['總賣費及稅']:,.0f} = 已實現損益 {s['總淨損益']:,.0f}　"
    f"（買手續費 {s['總買手續費']:,.0f} 已計入買進成本，勿再重複扣一次）"
)
